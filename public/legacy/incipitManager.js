...
var isChromePreviousCanvas; //20 01 2019
if (!!window.chrome && (!!window.chrome.webstore || !!window.chrome.runtime)) {
  isChromePreviousCanvas = true;
  isChrome = false;
}

drawingProblemYPatch = isFirefox || isChrome; //solves Drawing problem (Chrome and Firefox)
...
function CanvasClass() {
  ...
  this.setStepY = function (context) {
    ...
    var drawingProblemYPatchAmountFix = 0;
    if (drawingProblemYPatch) drawingProblemYPatchAmountFix = 0;
    context.stepY = size - drawingProblemYPatchAmountFix;
  };
  ...
  this.setDefaultClefAlt = function (
    context,
    clef,
    qtyAlteration,
    alterationName
    ...
    if (alterationName == "sostenido" || alterationName == "becuadro") {
    ...
    for (var i = index; i < context.drawIncipitElements.length; i++) {
      if (context.drawIncipitElements[i].qtyAlteration == 0) {
        context.drawIncipitElements[i].alterationName =
          context.defaultClefAlt[context.drawIncipitElements[i].yPosition];
        ...
        for (var j = i - 1; j > 0; j--) {
          ...
          if (
            yPosition == context.drawIncipitElements[j].yPosition &&
            context.drawIncipitElements[j].qtyAlteration > 0
          ) {
            context.drawIncipitElements[i].alterationName =
              context.drawIncipitElements[j].alterationName;
            break;
          }
        }
      }
  ...
  this.initializeCanvas = function (canvasElement, operation, paec) {
    ...
    if (operation != "list") {
      ...
      context.gCanvasElement.addEventListener(
        "mousemove",
        function (event) {
          context.hoverOnCanvas(context, event);
        },
        false
      );
      ...
      context.gCanvasElement.addEventListener(
        "click",
        function (event) {
          context.clickOnCanvas(context, event);
        },
        false
      );
    }
  ...
  this.showNote = function (context, cursor) {
    ...
    if (
      ...
      var tempEle = context.createElement(
        context, //insert
        note.name, //noteName
      ...
      var notePosition = context.getDrawPosition(
        context,
        tempEle.xPosition,
        tempEle.yPosition
      );
      ...
      if (tempEle.yPosition < 9 && !tempEle.isClef)
        tempFont = noteToDraw.value.toUpperCase();

      context.gDrawingContext.font = context.getFont(context, noteToDraw.font);
      context.gDrawingContext.fillText(
        tempFont,
        notePosition.x,
        notePosition.y
      );
      ...
      for (var i = 0; i < context.drawIncipitElements.length; i++) {
        if (
          ...
          context.drawIncipitElements[i].yPosition =
            context.drawIncipitElements[i].yPosition + up;
  ...
  this.addAlteration = function (context, currentAlteration, lastElement) {
    ...
    if (positionNoteSelected != null || lastElement == true) {
      ...
      if (context.drawIncipitElements[index].isClef) {
        ...
        if (alteration.name != "sostenido" && alteration.name != "bemol")
          return;
  ...
  this.addDot = function (context, lastElement) {
    if (positionNoteSelected != null || lastElement == true) {
      ...
      context.drawIncipitElements[index].hasDot =
        !context.drawIncipitElements[index].hasDot;
      context.updateDrawPosition(context, index);
      context.TransformIncipitToPAEC(context);
    }
  ...
  this.insertElement = function (
    ...
    context.drawIncipitElements.push(
  ...
  this.eraseNote = function (context) {
    ...
    if (positionNoteSelected != null && positionNoteSelected > 0) {
      ...
      for (
        var i = positionNoteSelected;
        i < context.drawIncipitElements.length;
        i++
      ) {
  ...
  this.getDrawPosition = function (context, elementX, elementY) {
    ...
    if (drawingProblemYPatch) {
      drawingProblemYPatchAmountFix = 3;
      contextMultiplier = 5;
    }
  ...
  this.drawPentagram = function (context) {
    ...
    for (var i = 0; i < context.drawIncipitElements.length; i++) {
      ...
      //Alteration
      drawingProblemYPatchAmountFix = 0;
      if (drawingProblemYPatch) drawingProblemYPatchAmountFix = 2;
      if (
        context.drawIncipitElements[i].qtyAlteration > 0 &&
        !noteToDraw.isRest
      ) {
        var alteration = context.incipit.getAlterationByName(
          context.drawIncipitElements[i].alterationName
        );
        ...
        if (context.drawIncipitElements[i].isClef) {
          for (
            var j = 0;
            j < context.drawIncipitElements[i].qtyAlteration;
            j++
          ) {
            ...
            if (alteration.name == "sostenido") {
              positionAlteration.x =
                context.incipit.AlterationClefPositionSostenido[j].xPosition;
              positionAlteration.y =
                context.incipit.AlterationClefPositionSostenido[j].yPosition;
            }

            if (alteration.name == "bemol") {
              positionAlteration.x =
                context.incipit.AlterationClefPositionBemol[j].xPosition;
              positionAlteration.y =
                context.incipit.AlterationClefPositionBemol[j].yPosition;
            }

            for (
              var k = 0;
              k < context.incipit.AlterationClefPositionNote.length;
              k++
            ) {
              if (
                context.incipit.AlterationClefPositionNote[k].name ==
                noteToDraw.name
              ) {
                diferentClef =
                  context.incipit.AlterationClefPositionNote[k].yPosition;
              }
            }

            noteAlteration = positionAlteration.x;
            context.gDrawingContext.font = context.getFont(
              context,
              alteration.font
            );
            context.gDrawingContext.fillText(
              ...
              notePosition.y +
                context.ratioY(context, diferentClef) +
                context.ratioY(
                  context,
                  positionAlteration.y - drawingProblemYPatchAmountFix
                )
            );
          }
        } else {
          ...
          context.gDrawingContext.fillText(
            ...
            notePosition.y +
              context.ratioY(
                context,
                alteration.yPosition - drawingProblemYPatchAmountFix
              )
          );
        }
      }
      ...
      //Time (only clef)
      if (
        context.drawIncipitElements[i].hasTime &&
        context.drawIncipitElements[i].isClef
      ) {
        drawingProblemYPatchAmountFix = 0;
        if (drawingProblemYPatch) drawingProblemYPatchAmountFix = 3.5;

        var time = context.incipit.getTimeByName(
          context.drawIncipitElements[i].timeName
        );

        var timePosition = context.getDrawPosition(
          context,
          context.drawIncipitElements[i].xPosition,
          time.yPosition + drawingProblemYPatchAmountFix
        );
      ...
      if (context.drawIncipitElements[i].hasDot) {
        ...
        context.gDrawingContext.fillText(
          dot.value,
          notePosition.x +
            context.ratioX(context, dot.xPosition - noteAlteration),
          notePosition.y + dot.yPosition - drawingProblemYPatchAmountFix
        );
      }

      //Bar
      if (
        context.drawIncipitElements[i].hasBar &&
        !context.drawIncipitElements[i].isClef
      ) {
        var bar = context.incipit.getBarByName(
          context.drawIncipitElements[i].barName
        );

        var barPosition = context.getDrawPosition(
          context,
          context.drawIncipitElements[i].xPosition,
          bar.yPosition
        );
        ...
        drawingProblemYPatchAmountFix = 0;
        if (drawingProblemYPatch) drawingProblemYPatchAmountFix = 6;

        context.gDrawingContext.font = context.getFont(
          context,
          bar.font + drawingProblemYPatchAmountFix
        );
        context.gDrawingContext.fillText(
          bar.value,
          barPosition.x +
            context.ratioX(context, bar.xPosition + noteDot - noteAlteration),
          barPosition.y
        );
      }
      ...
      if (context.drawIncipitElements[i].isClef) {
        noteAlteration = 0;
        if (drawingProblemYPatch) {
          drawingProblemYPatchAmountFix = 2;
          notePosition = context.getDrawPosition(
            context,
            context.drawIncipitElements[i].xPosition,
            context.drawIncipitElements[i].yPosition +
              drawingProblemYPatchAmountFix
          );
        }
      } else {
        context.noteNeedLine(
          context,
          notePosition.x - context.ratioX(context, noteAlteration),
          context.drawIncipitElements[i].yPosition
        );
      ...
      var tempFont = noteToDraw.value;
      if (
        context.drawIncipitElements[i].yPosition < 9 &&
        !context.drawIncipitElements[i].isClef
      )
        tempFont = noteToDraw.value.toUpperCase();
      ...
      context.gDrawingContext.fillText(
        tempFont,
        notePosition.x - context.ratioX(context, noteAlteration),
        notePosition.y + drawingProblemYPatchAmountFix
      );
    }
  ...
  this.getRythmPAEC = function (context, noteElement, note) {
    ...
    if (noteElement.hasDot) {
      paecRythm += context.incipit.getPAECByName(
        context.incipit.DotNote[0].name
      );
    }
  ...
  this.getAlterationPAEC = function (
    context,
    noteElement,
    lastArrayAlteration
  ) {
  ...
  this.TransformIncipitToPAEC = function (context) {
    ...
    for (var i = 0; i < context.drawIncipitElements.length; i++) {
      var note = context.incipit.getNoteByName(
        context.drawIncipitElements[i].noteName
      );
      var alteration = context.incipit.getAlterationByName(
        context.drawIncipitElements[i].alterationName
      ...
      if (context.drawIncipitElements[i].isClef) {
        ...
        paecRythm = context.getRythmPAEC(
          context,
        ...
        if (note.isRest) {
          ...
          var octaveRythm = context.getOctaveNotePAEC(
            context,
            context.drawIncipitElements[i],
            note.isRest,
            clef
        ...
        paecAlteration = context.getAlterationPAEC(
          context,
          context.drawIncipitElements[i],
          lastAlterationArray
        );
  ...
  this.TransformPAECToIncipit = function (context, paec) {
    ...
    for (var index = 0; index < paec.length; index++) {
      ...
      if (paec[index] == "'" || paec[index] == ",") {
        //octava
        var tempIndex = 0;

        while (
          index < paec.length &&
          (paec[index + tempIndex] == "'" || paec[index + tempIndex] == ",")
        ) {
          tempIndex++;
        }
      ...
      if (paec[index] == "x" || paec[index] == "b" || paec[index] == "n") {
        //ALTERATION de nota
      ...
      context.setDrawPosition(
        context,
        context.drawIncipitElements.length - 1,
        false
      );
